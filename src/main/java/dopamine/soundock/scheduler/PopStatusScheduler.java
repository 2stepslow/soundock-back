...
public class PopStatusScheduler {
    ...
    public void updatePopStatus(){
        ...
        try {
            ...
            // RECEIVED 처리 (현재 시간 - 3일 이전의 데이터)
            int receivedCount = popHistoryRepository.updateReceived(now.minusDays(3));
            ...
            if (donationCount > 0 || receivedCount > 0 || boardCount > 0) {
                log.info("pop 상태 처리 완료: DONATION: {}건, RECEIVED: {}건, FEATURED_BOARD: {}건", donationCount, receivedCount, boardCount);
            }
        } catch (Exception e) {
